...
void take_step(particle_t* me, system_t*system, unsigned int step){
...
void* run_simulation_thread(void *targ_in){
    ...
    for(step=0; step < system->nt; step++){
        ...
        for(i=0; i<targ->num_my_particles; i++){
            if(n==NULL) break;
            take_step(n->data,system,step);
            count++;
            n=n->next;
        }
        // block on the end barrier
        if(debug_flag) printf("[WORKER %i] completed step %i, processed %i particles\n",targ->thread_id,step,count);
